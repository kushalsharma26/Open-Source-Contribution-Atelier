...
@pytest.fixture
def admin_user(db):
    return User.objects.create_superuser(username="admin", email="admin@example.com", password="password123")


@pytest.mark.django_db
class TestIssueReportAPI:
    def test_submit_issue_anonymous_success(self, api_client):
        response = api_client.post("/api/issues/", {
    ...
    def test_submit_issue_with_image(self, api_client):
        ...
        image = SimpleUploadedFile("test_image.gif", valid_image_bytes, content_type="image/gif")
        response = api_client.post("/api/issues/", {
            "title": "UI overlap",
