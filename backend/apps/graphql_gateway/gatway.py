...
class GraphQLRouter:
    ...
    def _initialize_services(self):
        ...
        service_map = {
            'content': os.getenv('CONTENT_SERVICE_URL', 'http://localhost:8001/graphql'),
            'progress': os.getenv('PROGRESS_SERVICE_URL', 'http://localhost:8002/graphql'),
            'users': os.getenv('USERS_SERVICE_URL', 'http://localhost:8003/graphql'),
            'chat': os.getenv('CHAT_SERVICE_URL', 'http://localhost:8004/graphql'),
            'notifications': os.getenv('NOTIFICATIONS_SERVICE_URL', 'http://localhost:8005/graphql'),
        }
    ...
    def route_query(self, query: str, variables: Optional[Dict] = None) -> Dict[str, Any]:
        """
    ...
    def _determine_services(self, query: str) -> List[str]:
        ...
        # Content service
        if any(field in query for field in ['content', 'contents', 'module', 'modules', 'lesson']):
        ...
        # Chat service
        if any(field in query for field in ['conversation', 'conversations', 'message', 'messages']):
            services.append('chat')
        
        # Notifications service
        if any(field in query for field in ['notification', 'notifications', 'unreadCount']):
            services.append('notifications')
        
        return services or ['content']  # Default to content
    
    def _execute_on_service(self, service: str, query: str, variables: Optional[Dict] = None) -> Dict[str, Any]:
        """
...
class FederationQueryPlanner:
    ...
    def plan(self, query: str, variables: Optional[Dict] = None) -> List[Dict[str, Any]]:
        """
