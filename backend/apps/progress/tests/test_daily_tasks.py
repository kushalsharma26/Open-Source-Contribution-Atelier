...
from apps.progress.models import DailyTaskRecord, LessonProgress, PeerReview, QuizAttempt, CodeSubmission
...
class TestDailyTasks:
    ...
    def test_lesson_daily_task(self, user, lesson):
        ...
        LessonProgress.objects.create(user=user, lesson=lesson2, completed=True, score=100)
    ...
    def test_pr_daily_task(self, user, other_user):
        submission = CodeSubmission.objects.create(user=other_user, title="Test PR", code_snippet="print(1)")
        PeerReview.objects.create(submission=submission, reviewer=user, feedback="Good", rating=5)
    ...
    def test_quiz_daily_task(self, user):
        QuizAttempt.objects.create(user=user, question_id="q1", is_correct=True, selected_answer="A", correct_answer="A")
    ...
    def test_edge_case_double_awarding(self, user, lesson):
        ...
        LessonProgress.objects.create(user=user, lesson=lesson3, completed=True, score=100)
    ...
    def test_edge_case_incorrect_quiz_no_award(self, user):
        ...
        QuizAttempt.objects.create(user=user, question_id="q2", is_correct=False, selected_answer="B", correct_answer="A")
        
        # record might not even exist if no correct quizzes or other tasks
        record = DailyTaskRecord.objects.filter(user=user, date=timezone.localdate()).first()
        if record:
    ...
    def test_edge_case_lesson_recompletion(self, user, lesson):
        ...
        lp = LessonProgress.objects.create(user=user, lesson=lesson, completed=True, score=100)
        record = DailyTaskRecord.objects.get(user=user, date=timezone.localdate())
        assert record.lessons_completed == 1
    ...
    def test_edge_case_cross_day_boundary(self, user, other_user):
        ...
        record_yesterday = DailyTaskRecord.objects.create(user=user, date=yesterday, prs_reviewed=1, prs_awarded=True, xp_earned=15)
        
        # Today's PR review
        submission = CodeSubmission.objects.create(user=other_user, title="PR", code_snippet="print(2)")
        PeerReview.objects.create(submission=submission, reviewer=user, feedback="Good", rating=5)
    ...
    def test_edge_case_lesson_recompletion_no_update_fields(self, user, lesson):
        ...
        record.refresh_from_db()
        assert record.lessons_completed == 1 # Should still be 1 if signal handles it correctly
